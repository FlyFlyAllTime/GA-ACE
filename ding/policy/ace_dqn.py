...
class SMACACEDQNPolicy(Policy):
    ...
    def _forward_learn(self, data: dict) -> Dict[str, Any]:
        ...
        #这一块是自己加的
        #reward_zsy=reward/agent_num*torch.ones(agent_num)
        reward_zsy=sum(reward)/len(reward)*torch.ones(len(reward))

            
        target = target_q
        next_target = torch.mm(torch.cat([reward_zsy, next_target_q], dim=1),
                               self._gamma ** torch.arange(0, self._nstep + 1).unsqueeze(1).to(reward_zsy))
