    ...
    def eval(
        ...
        with self._timer:
            while not eval_monitor.is_finished():
                ...
                timesteps = self._env.step(actions)#这里_env是\dizoo\smac\envs\smac_env_ace
                timesteps = to_tensor(timesteps, dtype=torch.float32)
                for env_id, t in timesteps.items():#这里t是timesteps的内容，(obs,reward,done,info,episode_steps)
                    ...
                    if t.done:
                        ...
                        self._policy.reset([env_id])
                        reward = t.info['final_eval_reward']#这里的reward是0或1
